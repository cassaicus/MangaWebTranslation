...
class Coordinator: NSObject, WKNavigationDelegate {
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        // 親ビューに定義されたコールバックを呼び出し、現在のURLを渡します。
...
struct CrossPlatformWebView: View {
    ...
    init() {
        // UserDefaultsから保存された初期URLを読み込み、状態変数を初期化します。
        let initialUrl = UserDefaults.standard.string(forKey: "initialUrl") ?? "https://www.google.com"
    ...
    private var contentView: some View {
        ...
        .onAppear {
            ...
            if #available(macOS 14.0, iOS 17.0, *) {
                // 翻訳のソース言語（日本語）とターゲット言語（英語）を設定します。
    ...
    /// UserDefaultsから初期URLを読み込み、URL入力フィールドのテキストを更新します。
    private func loadInitialUrl() {
        let savedUrl = UserDefaults.standard.string(forKey: "initialUrl") ?? "https://www.google.com"
